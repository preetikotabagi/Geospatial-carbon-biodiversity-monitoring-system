    ...
    try {
      ...
      setError(
        err.response?.data?.detail ||
          "Failed to save this year's metrics."
      );
    } finally {
      setSaving(false);
    }
...
function AnalyticsPanel({
  ...
  /*
   * Whenever a site is selected and analytics are loaded,
   * automatically move the page to the analytics section.
   */
  useEffect(() => {
    if (site && !loading && analyticsRef.current) {
      setTimeout(() => {
  ...
  if (error && !site && !loading) {
    return (
      <div
        ref={analyticsRef}
        className="analytics-panel analytics-error-panel"
        style={{
          gridColumn: "1 / -1",
          width: "100%",
        }}
      >
        ...
        <p className="hint-text">
          The map and site data are still available. You can select another
          site or try again after checking the API connection.
        </p>
      </div>
    );
  }
  ...
  const carbonChartData = {
    ...
    datasets: [
      {
        label: "Carbon (tCO₂e)",
        data: (site.history || []).map(
          (item) => item.carbon_tco2e
        ),
        borderColor: "#1f7a4c",
    ...
    >
      ...
      <div className="analytics-metrics">
        ...
        <div className="analytics-metric">
          ...
          <p className="value">
            {site.current.biodiversity_score}
          </p>
      ...
      {hasHistory ? (
        ...
        <div className="empty-state empty-state-panel">
          No historical metrics recorded for this site yet. Add one
          below to see it plotted on the performance charts.
        </div>
      )}

      <div className="metric-form-wrapper">
        <p className="hint-text metric-form-hint">
          Carbon and biodiversity figures are illustrative demo values
          for this MVP, entered manually below &mdash; only{" "}
          <strong>area</strong> is a real, computed measurement.
        </p>
